...
**Cockpit** (Phase 2):
The local browser UI over the workflow. Renders a left-nav project picker, and per project shows the agent profiles, the action queue, the task list with a **rich detail panel** (the plan, the implementation-log timeline, and the per-role **Run-token** breakdown), wiki status, governance, and timesheet status — all projected from the project's `.tcgstackflow/` files. **No longer read-only** — superseding the original Phase-2 stance (ADR 0020/0023): the Cockpit writes the canonical task files directly (a manual **Status override**, auto-logged) and **launches agent Runs** — it has grown into the **Orchestrator**. The one load-bearing invariant that survives the reversal is *files are the single source of truth — no second store*: the Cockpit writes the canonical task files and the `runs/` area, never a parallel database. _Avoid_: dashboard, console (too generic), web app.
