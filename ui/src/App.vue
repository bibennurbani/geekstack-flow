...
function openReportHtmlFor(pp, id, runId) {
  window.open(
    '/api/project/task/report.html?path=' +
...
async function openRunGlobal(pp, id, runId) {
  ...
  runView.value = null;
  runViewProject.value = pp;
  runView.value = await api(
    '/api/project/task/run?path=' +
      encodeURIComponent(pp) +
      '&id=' +
      encodeURIComponent(id) +
      '&run=' +
...
function copyPrompt(taskId, agent, key) {
  const txt =
    `Adopt the ${agent} role per .tcgstackflow/agents/${agent}.md and work on ${taskId}. ` +
...
// --- task detail panel ---
const taskUrl = () =>
  '/api/project/task?path=' +
  encodeURIComponent(selected.value) +
  '&id=' +
  encodeURIComponent(selectedTask.value);
async function openTask(t) {
  ...
  isolationMode.value =
    (detail.value &&
      detail.value.config &&
      detail.value.config.orchestrator &&
      detail.value.config.orchestrator.isolation) ||
    'in-place';
  ...
  if (ar && ar.run_id) {
    subscribeRun(ar.run_id);
    toast('Reattached to the running ' + (ar.role || 'agent'), 'info');
  }
}
function closeTask() {
...
async function refreshTask() {
  if (selectedTask.value) taskDetail.value = await api(taskUrl());
}
...
async function changeStatus(e) {
  ...
  const res = await postJSON('/api/project/task/status', {
    path: selected.value,
    id: selectedTask.value,
    status,
  });
  if (!res.ok) {
    statusError.value = `Status write failed (${res.status})`;
    e.target.value = prev;
    await refreshTask();
    return;
  }
...
function resetRun() {
  ...
  runId.value = '';
  streamText.value = '';
  liveTokens.value = null;
  runError.value = '';
  pendingApproval.value = null;
  criticalAck.value = false;
  runBranch.value = '';
}
...
function subscribeRun(id) {
  ...
  es.addEventListener('delta', (ev) => {
    streamText.value += JSON.parse(ev.data).text || '';
  });
  es.addEventListener('tokens', (ev) => {
    liveTokens.value = JSON.parse(ev.data);
  });
  es.addEventListener('isolation', (ev) => {
    const d = JSON.parse(ev.data);
    runBranch.value = d.branch || '';
  }); // ADR 0040 — branch this run created/continued

  es.addEventListener('approval_request', (ev) => {
    pendingApproval.value = JSON.parse(ev.data);
    criticalAck.value = false;
  ...
  es.addEventListener('status', (ev) => {
    ...
    if (d.state === 'error') {
      ...
      pendingApproval.value = null;
      runError.value = d.reason || d.error || 'exit ' + (d.code ?? '?');
      toast('Run failed: ' + runError.value, 'err');
    } else if (d.state === 'queued') {
      runError.value = 'queued — waiting for the project slot';
    } else if (d.state === 'aborting') {
      runError.value = 'stopping…';
    } else if (d.state === 'aborted') {
      runState.value = 'error';
      pendingApproval.value = null;
      runError.value = 'stopped by you';
      closeStream();
      refreshTask();
      toast('Run stopped');
    }
  ...
  es.addEventListener('chain', async (ev) => {
    ...
    if (d.state === 'next') {
      ...
      try {
        ...
        subscribeRun(d.run_id);
      } // the hop must never be dropped
    } else if (d.state === 'done') {
      toast('Chain complete — ' + prettyStatus(d.status || ''), 'ok');
    } else if (d.state === 'ready-for-pr') {
      streamText.value += `\n\n━━ ready for PR: ${d.branch} — use the ⑂ PR button to review & open ━━\n\n`;
      toast('Ready for PR: ' + d.branch, 'ok');
      refreshTask();
    } else if (d.state === 'stopped') {
      toast(
        'Chain stopped: ' +
          (d.reason === 'bounce-limit'
            ? `bounce limit (${d.next_ready} still ready)`
            : d.reason),
        'err',
      );
    }
...
async function startRun(force = false) {
  ...
  const res = await postJSON('/api/run', {
    project_path: selected.value,
    task_id: selectedTask.value,
    role,
    force,
    chain: chainOn.value || undefined,
    isolation: isolationMode.value,
  });
  if (!res.ok) {
    ...
    if (
      ...
      !force &&
      window.confirm(
        `Est. spend $${j.spend} exceeds the $${j.budget} budget. Run anyway?`,
      )
    ) {
    ...
    runState.value = 'error';
    runError.value =
      j.error === 'governance-gate-not-ready'
        ? 'Governance gate not ready'
        : j.error === 'over-budget'
          ? `Over budget ($${j.spend} ≥ $${j.budget}) — raise it in Settings`
          : j.error === 'task-already-running'
...
async function quickRun(projectPath, taskId, role, chain = false) {
  ...
  if (res.ok) {
    ...
    } else if (selected.value === null && !showRuns.value) {
      agents.value = await api('/api/agents');
    }
  } else
    toast(
      j.error === 'task-already-running'
        ? taskId + ' already has an active run'
        : 'Launch failed: ' + (j.error || res.status),
      'err',
    );
}
...
async function reviewPr(taskId) {
  prResult.value = null;
  prBusy.value = true;
  prPanel.value = { task_id: taskId };
  const j = await api(
    '/api/task/pr/plan?path=' +
      encodeURIComponent(selected.value) +
      '&id=' +
  ...
  prPanel.value =
    j && !j.error
      ? { task_id: taskId, ...j }
      : { task_id: taskId, error: (j && j.error) || 'plan failed' };
}
function closePr() {
  prPanel.value = null;
  prResult.value = null;
...
async function confirmPr() {
  ...
  prBusy.value = true;
  const res = await postJSON('/api/task/pr', {
    path: selected.value,
    id: prPanel.value.task_id,
  });
  const j = await res.json().catch(() => ({}));
  prBusy.value = false;
  if (res.ok) {
    prResult.value = j;
    toast(j.pr_url ? 'PR opened' : 'Branch pushed — open the PR', 'ok');
...
async function openWorktrees() {
  wtBusy.value = true;
  wtPanel.value = { worktrees: [] };
  const j = await getWorktrees(selected.value);
  wtBusy.value = false;
  wtPanel.value =
    j && !j.error ? j : { worktrees: [], error: (j && j.error) || 'listing failed' };
}
...
async function startAll() {
  const q = ((detail.value && detail.value.action_queue) || []).filter(
    (a) => !a.run_state,
  );
  if (!q.length) {
    toast('Nothing ready to start', 'info');
    return;
  }
...
async function decide(decision) {
  ...
  if (
    pendingApproval.value.risk === 'CRITICAL' &&
    decision === 'approve' &&
    !criticalAck.value
  )
    return;
  await postJSON('/api/run/approval', {
    run_id: runId.value,
    approval_id: pendingApproval.value.approval_id,
    decision,
    ack: criticalAck.value,
  });
...
async function openReport(runId = null) {
  ...
  report.value = null;
  reportRun.value = runId || null;
  report.value = await api(
    '/api/project/task/report?path=' +
      encodeURIComponent(selected.value) +
      '&id=' +
      encodeURIComponent(selectedTask.value) +
...
async function openRun(runId) {
  runLoading.value = true;
  runView.value = null;
  runViewProject.value = selected.value;
  runView.value = await api(
    '/api/project/task/run?path=' +
      encodeURIComponent(selected.value) +
      '&id=' +
...
const chatSession = computed(() => {
  ...
  const runs =
    (taskDetail.value && taskDetail.value.tokens && taskDetail.value.tokens.runs) || [];
...
async function sendChat() {
  ...
  const res = await postJSON('/api/run/message', {
    project_path: selected.value,
  ...
  if (!res.ok) {
    const j = await res.json().catch(() => ({}));
    chatMessages.value[idx].text =
      j.error === 'project-busy'
        ? '[project busy — wait for the active run to finish]'
        : '[failed to start — ' + (j.error || res.status) + ']';
    chatBusy.value = false;
    return;
  }
  ...
  chatEs.addEventListener('delta', (ev) => {
    chatMessages.value[idx].text += JSON.parse(ev.data).text || '';
  });
  ...
  chatEs.addEventListener('status', (ev) => {
    const d = JSON.parse(ev.data);
    if (d.state === 'error') {
      chatMessages.value[idx].text +=
        (chatMessages.value[idx].text ? '\n' : '') +
...
const hasRuns = (t) => {
  const x = t && t.tokens_total;
  return !!x && x.input + x.output + x.cache_read + x.cache_creation > 0;
};
function generateAnalysis() {
  const sids = ((report.value && report.value.sessions) || [])
    .filter((s) => s.found)
    .map((s) => s.session_id);
  const logs = sids.length
    ? sids.map((s) => '~/.claude/projects/*/' + s + '.jsonl').join(', ')
    : '(no session logs found on this machine)';
  const txt =
    `Author a session-telemetry post-mortem as a self-contained dark-themed HTML report (style of a "Where the tokens went" / session_report) for task ${selectedTask.value} in ${selected.value}. ` +
  ...
  setTimeout(() => {
    if (copiedKey.value === 'report') copiedKey.value = '';
  }, 1800);
}
const costRows = computed(() => {
  if (!report.value) return [];
  const t = report.value.totals.tokens,
    c = report.value.totals.cost.by_class,
    total = report.value.totals.cost.total || 1e-9;
  const rows = [
  ...
  return rows
    ...
    .map((r) => ({
      ...
      w: Math.max(0.8, (r.usd / max) * 100),
    }));
});
const toolMax = computed(() =>
  report.value && report.value.tools_by_type[0] ? report.value.tools_by_type[0].count : 1,
);
const tracePath = computed(() => {
  const tl = (report.value && report.value.timeline) || [];
  if (!tl.length) return null;
  const max = Math.max(...tl.map((p) => p.cache_read), 1),
    n = tl.length;
  const pts = tl.map((p, i) => {
    const x = n === 1 ? 1000 : (i / (n - 1)) * 1000;
    const y = 150 - (p.cache_read / max) * 138;
    return x.toFixed(1) + ',' + y.toFixed(1);
  });
  return {
    line: 'M' + pts.join(' L'),
    area: 'M0,150 L' + pts.join(' L') + ' L1000,150 Z',
  };
});
const wallClock = computed(() => {
  const ms = (report.value && report.value.totals.wall_clock_ms) || 0;
...
function toggleSort(col) {
  taskSort.value =
    taskSort.value.col === col ? { col, dir: -taskSort.value.dir } : { col, dir: 1 };
}
function filterByAgent(role) {
  taskAgent.value = role;
  taskBucket.value = 'active';
  projectTab.value = 'tasks';
}
const projectTasks = computed(() => (detail.value && detail.value.tasks) || []);
const taskStatuses = computed(() =>
  [...new Set(projectTasks.value.map((t) => t.status))].sort(),
);
const bucketCounts = computed(() => bucketCountsOf(projectTasks.value));
const filteredTasks = computed(() =>
  filterTasks(projectTasks.value, {
    bucket: taskBucket.value,
...
const agentList = computed(() =>
  agents.value
    ? (agents.value.order || ROLE_ORDER).map((r) => agents.value.roles[r]).filter(Boolean)
    : [],
);
const activeAgents = computed(() =>
  agentList.value.filter(
    (a) => (a.queue && a.queue.length) || a.runs || tokSum(a.tokens) > 0,
  ),
);
const queueGroups = computed(() =>
  agentList.value.filter((a) => a.queue && a.queue.length),
);
const readyCount = computed(() =>
  agentList.value.reduce((n, a) => n + (a.queue ? a.queue.length : 0), 0),
);
const agentDetail = computed(() =>
  agents.value && selectedAgent.value ? agents.value.roles[selectedAgent.value] : null,
);
...
const homeTotals = computed(() => {
  ...
  return {
    tokens: tk.input + tk.output + tk.cache_read + tk.cache_creation,
    cost: costOfTokens(tk),
    runs,
    activeAgents: active,
  };
});
...
const projectAgents = computed(() => {
  ...
  return ROLE_ORDER.map((r) => m[r]).filter(
    (a) => a && (a.queue || a.runs || tokSum(a.tokens) > 0),
  );
});
...
async function pollInbox() {
  try {
    ...
    /* server briefly down — keep polling */
  }
}
function notifyApproval(a) {
  ...
  try {
    if (typeof Notification !== 'undefined' && Notification.permission === 'granted')
      new Notification('GeekStack Flow — approval needed', {
        body: `${a.risk}: ${a.action}`,
      });
  } catch {
...
function openInbox() {
  // nav click = a user gesture — the one place a permission request works
...
async function decideInbox(a, decision) {
  const res = await postJSON('/api/run/approval', {
    run_id: a.run_id,
    approval_id: a.approval_id,
    decision,
    ack: !!inboxAck.value[a.approval_id],
  });
  if (!res.ok) {
    const j = await res.json().catch(() => ({}));
    toast(
      j.error === 'already-resolved'
        ? `Already resolved (${j.decision}) — your click had no effect`
        : j.error === 'critical-ack-required'
          ? 'Acknowledge the rollback to approve a CRITICAL action'
          : j.error === 'unknown-approval'
            ? 'No longer pending — the run ended'
            : 'Decision failed: ' + (j.error || res.status),
      'err',
    );
  }
  await pollInbox();
...
async function openDiff(runId) {
  ...
  diffView.value = null;
  const d = await api(
    '/api/project/task/run/diff?path=' +
      encodeURIComponent(selected.value) +
      '&id=' +
      encodeURIComponent(selectedTask.value) +
      '&run=' +
...
const projectBudget = computed(() =>
  detail.value && detail.value.config && detail.value.config.orchestrator
    ? detail.value.config.orchestrator.budget_usd
    : null,
);
...
const overBudget = computed(
  () => projectBudget.value != null && projectSpend.value > projectBudget.value,
);

const updateCount = computed(
  () => projects.value.filter((p) => p.update_available).length,
);
...
function wikiDiscoveryTitle(w) {
  ...
  if (w.path === 'qmd')
    parts.push(
      `Discovered via qmd${w.queries ? ` (${w.queries} ${w.queries === 1 ? 'query' : 'queries'})` : ''}`,
    );
  else if (w.path === 'index-fallback')
    parts.push(`index.md Map-of-Content fallback${w.reason ? ` (qmd ${w.reason})` : ''}`);
  if (w.redirects)
    parts.push(
      `${w.redirects} pre-qmd wiki grep${w.redirects === 1 ? '' : 's'} redirected to qmd`,
    );
  return parts.join(' · ');
}
const roleEntries = computed(() =>
  taskDetail.value && taskDetail.value.tokens
    ? Object.entries(taskDetail.value.tokens.by_role || {})
    : [],
);
const timelineNewest = computed(() =>
  taskDetail.value ? [...(taskDetail.value.timeline || [])].reverse() : [],
);

onMounted(async () => {
  try {
    const pr = await api('/api/pricing');
    if (pr && pr.pricing) pricing.value = pr.pricing;
  } catch {
...
<template>
  <div class="layout">
    <nav>
      ...
      <div
        class="nav-item"
        :class="{ active: selected === null && !showRuns }"
        @click="loadHome">
      ...
      <div
        ...
        <span
          ...
          class="nav-dot"
          title="Workspace update available"></span>
      </div>
      <div v-if="!projects.length" class="muted" style="padding: 8px; font-size: 12px">
        No projects yet. Run
        <code style="color: inherit; background: rgba(255, 255, 255, 0.08); border: 0"
          >geekstackflow init</code
        >.
      </div>
      <div class="nav-note">
      ...
      <div class="content">
        ...
        <template v-else-if="showRuns">
          <div class="page-head">
            ...
            <div class="meta">
              <span
                >{{ runsHistory.length }} run{{
                  runsHistory.length === 1 ? '' : 's'
                }}
                across the workspace · newest first</span
              >
            </div>
          </div>
          <div v-if="!runsHistory.length" class="empty">
            No orchestrated runs yet. Launch one with the <b>Run</b> button on a task.
          </div>
          <div v-else class="ttable">
            <div class="tr th" style="grid-template-columns: 1fr 120px 92px 88px auto">
              <span>Task</span><span>Project</span><span>Role</span><span>State</span
              ><span></span>
            </div>
            <div
              v-for="r in runsHistory"
              :key="r.project_path + r.run_id"
              class="tr trow"
              style="grid-template-columns: 1fr 120px 92px 88px auto"
              @click="openRunGlobal(r.project_path, r.task_id, r.run_id)">
              <span class="tc-task"
                ><b class="task-id">{{ r.task_id }}</b>
                <span class="mono muted" style="font-size: 11px"
                  >{{ r.run_id.slice(0, 8) }}…</span
                ></span
              >
              <span
                class="muted"
              ...
              >
              <span
                ><span class="agent" :class="'agent-' + r.role">{{ r.role }}</span></span
              >
              <span
                ><span
                  class="badge"
              ...
              <span class="tc-act">
                ...
                <button
                  ...
                  :class="{ 'btn-copied': copiedKey === 'hterm' + r.run_id }"
                  title="Resume this session in your terminal"
                  @click.stop="
                    copyResume(r.project_path, r.session_id, 'hterm' + r.run_id)
                  ">
                  {{ copiedKey === 'hterm' + r.run_id ? '✓' : '⌥ term' }}
                </button>
                <button
                  class="btn"
                  style="padding: 3px 9px; font-size: 11px"
                  @click.stop="openReportHtmlFor(r.project_path, r.task_id, r.run_id)">
                  report ↗
                </button>
              </span>
            </div>
        ...
        <template v-else-if="selected === null">
          ...
          <template v-if="selectedAgent && agentDetail">
            <div class="detail-head">
              ...
              <h1 style="text-transform: capitalize">
                <span class="agent" :class="'agent-' + agentDetail.role">{{
                  (agentDetail.profile && agentDetail.profile.name) || agentDetail.role
                }}</span>
              </h1>
            </div>
            <p
              v-if="agentDetail.profile && agentDetail.profile.role"
              class="muted"
              style="margin: -6px 0 12px; font-size: 14px">
              {{ agentDetail.profile.role }}
            </p>

            <div class="tabs">
              <button
                class="tab"
                :class="{ active: agentTab === 'queue' }"
                @click="agentTab = 'queue'">
                Queue <span class="tab-n">{{ agentDetail.queue.length }}</span>
              </button>
              <button
                class="tab"
                :class="{ active: agentTab === 'profile' }"
                @click="agentTab = 'profile'">
                Profile
              </button>
              <button
                class="tab"
                :class="{ active: agentTab === 'tokens' }"
                @click="agentTab = 'tokens'">
                Tokens
              </button>
            </div>
            ...
            <template v-if="agentTab === 'queue'">
              <div v-if="!agentDetail.queue.length" class="empty">
                Nothing waiting for this agent.
              </div>
              <div
                v-for="(q, i) in agentDetail.queue"
                :key="q.project_path + q.task_id"
                class="card row interactive"
                @click="openTaskInProject(q.project_path, q.task_id)">
                <div class="grow">
                  <span class="task-id">{{ q.task_id }}</span
                  ><span class="task-title">{{ q.title }}</span>
                  <div class="sub">
                    <span class="badge soft">{{ q.project }}</span
                    ><span class="badge" :class="'st-' + q.status">{{
                ...
                <button
                  ...
                  title="Launch this agent now"
                  @click.stop="quickRun(q.project_path, q.task_id, agentDetail.role)">
                  ▶
                </button>
                <button
                  class="btn"
                  :class="{ 'btn-copied': copiedKey === 'a' + i }"
                  @click.stop="copyPrompt(q.task_id, agentDetail.role, 'a' + i)">
                  {{ copiedKey === 'a' + i ? '✓ Copied' : 'Copy prompt' }}
                </button>
              </div>
            </template>
            ...
            <template v-else-if="agentTab === 'profile'">
              <p
                v-if="agentDetail.profile && agentDetail.profile.description"
                style="max-width: 72ch; color: var(--text-2)">
                {{ agentDetail.profile.description }}
              </p>
              <div
                v-if="agentDetail.profile && agentDetail.profile.skills.length"
                class="chips"
                style="margin: 14px 0 4px">
                <span class="muted" style="font-size: 12px">Skills used</span>
                <span
                  v-for="s in agentDetail.profile.skills"
                  :key="s"
                  class="badge soft"
                  >{{ s }}</span
                >
              </div>
              <div v-if="!agentDetail.profile" class="empty">
            ...
            <template v-else-if="agentTab === 'tokens'">
              <p class="muted" style="font-size: 12.5px; margin: 0 0 14px">
                {{ agentDetail.runs }} run{{ agentDetail.runs === 1 ? '' : 's' }} across
                {{ agentDetail.projects }} project{{
                  agentDetail.projects === 1 ? '' : 's'
                }}
                · ~{{ fmtUsd(costOfTokens(agentDetail.tokens)) }} est.
              </p>
              <div v-if="tokSum(agentDetail.tokens)" class="agent-tok">
                <div>
                  <div class="v">{{ fmtTok(agentDetail.tokens.input) }}</div>
                  <div class="k">input</div>
                </div>
                <div>
                  <div class="v">{{ fmtTok(agentDetail.tokens.output) }}</div>
                  <div class="k">output</div>
                </div>
                <div>
                  <div class="v">{{ fmtTok(agentDetail.tokens.cache_read) }}</div>
                  <div class="k">cache read</div>
                </div>
                <div>
                  <div class="v">{{ fmtTok(agentDetail.tokens.cache_creation) }}</div>
          ...
          <template v-else>
            <div class="hero">
              <div>
                <div class="hero-eyebrow">
                  orchestration · {{ projects.length }} project{{
                    projects.length === 1 ? '' : 's'
                  }}
                </div>
                <h1 class="hero-h1">
                  {{ readyCount }} task{{ readyCount === 1 ? '' : 's' }} ready<br /><span
                    class="hero-em"
                    >across your workspace</span
                  >
                </h1>
              </div>
              <div
                class="hero-big"
                title="Estimate from recorded run totals (Opus list pricing). Open a task's Session report for precise per-turn cost.">
            ...
            <div class="hero-metrics">
              ...
              <div>
                <div class="v">{{ homeTotals.activeAgents }}</div>
                <div class="k">agents active</div>
              </div>
              <div>
                <div class="v">{{ homeTotals.runs }}</div>
                <div class="k">runs</div>
              </div>
              <div>
                <div class="v">{{ fmtTok(homeTotals.tokens) }}</div>
                <div class="k">tokens</div>
              </div>
              <div>
                <div class="v" :style="{ color: updateCount ? 'var(--amber)' : '' }">
                  {{ updateCount }}
                </div>
            ...
            <div class="agent-cards">
              <div
                ...
                class="agent-card"
                :class="'ac-' + a.role"
                @click="openAgent(a.role)">
                <div class="ac-name agent" :class="'agent-' + a.role">
                  {{ (a.profile && a.profile.name) || a.role }}
                </div>
                <div class="ac-stat">
                  <b>{{ a.queue.length }}</b> ready
                </div>
                <div class="ac-sub muted">
                  {{ a.runs }} run{{ a.runs === 1 ? '' : 's' }} ·
                  {{ fmtTok(tokSum(a.tokens)) }} tok
                </div>
              </div>
            </div>
            <div v-if="!activeAgents.length" class="empty">
              No agents have work or runs yet. ✓
            </div>
            ...
            <template v-for="g in queueGroups" :key="g.role">
              <div class="section agent-group-head" @click="openAgent(g.role)">
                <span
                  ...
                  >{{ (g.profile && g.profile.name) || g.role }}</span
                >
              ...
              <div
                ...
                <button
                  ...
                  style="padding: 5px 11px"
                  title="Launch this agent now"
                  @click.stop="quickRun(q.project_path, q.task_id, g.role)">
                  ▶
                </button>
                <button
                  class="btn"
                  :class="{ 'btn-copied': copiedKey === g.role + i }"
                  @click.stop="copyPrompt(q.task_id, g.role, g.role + i)">
                  {{ copiedKey === g.role + i ? '✓ Copied' : 'Copy prompt' }}
                </button>
              </div>
            </template>
          </template>
          ...
          <template v-if="selectedTask && taskDetail">
            ...
            <template v-if="showReport">
              <div class="detail-head">
                ...
                <h1>
                  Session report · <span class="task-id">{{ taskDetail.id }}</span>
                </h1>
                <span v-if="reportRun" class="badge soft mono"
                  >run {{ reportRun.slice(0, 8) }}…</span
                >
                <span class="grow"></span>
                <button
                  class="btn"
                  :class="{ 'btn-copied': copiedKey === 'report' }"
                ...
                <button
                  class="btn btn-primary"
                  @click="openReportHtml(reportRun)"
                  title="Open the standalone HTML report in a new tab">
                  Open report ↗
                </button>
              </div>
              ...
              <template v-else-if="report && !report.error">
                <div class="sreport">
                  <div class="sr-hero">
                    <div>
                      <div class="sr-eyebrow">
                        token telemetry · {{ report.sessions_found }}/{{
                          report.sessions.length
                        }}
                        session{{ report.sessions.length === 1 ? '' : 's' }} found
                      </div>
                      <div class="sr-tokens-processed">
                        {{ fmtTok(report.totals.tokens_processed) }}
                        <span>tokens processed</span>
                      </div>
                      <div class="muted" style="font-size: 12.5px; margin-top: 4px">
                        model {{ report.model || '—' }} · {{ report.totals.turns }} turns
                  ...
                  <div class="sr-metrics">
                    <div>
                      <div class="v">{{ wallClock }}</div>
                      <div class="k">wall-clock</div>
                    </div>
                    <div>
                      <div class="v">{{ report.sessions.length }}</div>
                      <div class="k">runs</div>
                    </div>
                    <div>
                      <div class="v">{{ report.totals.tool_calls }}</div>
                      <div class="k">tool calls</div>
                    </div>
                    <div>
                      <div class="v">{{ fmtTok(report.totals.tokens_processed) }}</div>
                      <div class="k">tokens</div>
                    </div>
                    <div>
                      <div
                        class="v"
                        :style="{
                          color: report.totals.mcp_calls ? '' : 'var(--coral2)',
                        }">
                        {{ report.totals.mcp_calls }}
                      </div>
                      <div class="k">MCP calls</div>
                    </div>
                  </div>

                  <div class="sr-sec-head"><h2>Where the tokens went</h2></div>
                  <div class="sr-tgrid">
                    <div class="sr-tcard hot">
                      <div class="v">{{ fmtTok(report.totals.tokens.cache_read) }}</div>
                      <div class="n">Cache reads</div>
                      <div class="s">Context re-read each turn</div>
                    </div>
                    <div class="sr-tcard">
                      <div class="v">
                        {{ fmtTok(report.totals.tokens.cache_creation) }}
                      </div>
                      <div class="n">Cache writes</div>
                      <div class="s">New context committed to cache</div>
                    </div>
                    <div class="sr-tcard">
                      <div class="v">{{ fmtTok(report.totals.tokens.output) }}</div>
                      <div class="n">Output</div>
                      <div class="s">Tokens generated</div>
                    </div>
                    <div class="sr-tcard">
                      <div class="v">{{ fmtTok(report.totals.tokens.input) }}</div>
                      <div class="n">Fresh input</div>
                      <div class="s">Uncached prompt tokens</div>
                    </div>
                  </div>
                  <div class="sr-wf">
                    <div v-for="r in costRows" :key="r.name" class="sr-wf-row">
                      <div class="sr-wf-head">
                        <span class="sr-wf-name"
                          >{{ r.name }}
                          <span v-if="r.hot" class="sr-flag">cost driver</span></span
                        >
                        <span class="sr-wf-val"
                          >{{ fmtUsd(r.usd) }}
                          <span class="sr-wf-pct">{{ r.pct }}%</span></span
                        >
                      </div>
                      <div class="sr-bar">
                        <span
                          :style="{
                            width: r.w + '%',
                            background: r.hot
                              ? 'linear-gradient(90deg,var(--amber2),var(--coral2))'
                              : '#34416a',
                          }"></span>
                      </div>
                      <div class="sr-wf-sub">
                        {{ Number(r.tok).toLocaleString() }} tokens
                      </div>
                    </div>
                  </div>
                  ...
                  <div v-if="tracePath" class="sr-trace">
                    <svg viewBox="0 0 1000 150" preserveAspectRatio="none">
                      <defs>
                        <linearGradient id="srg" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="0" stop-color="#f25c54" stop-opacity=".5" />
                          <stop offset="1" stop-color="#f25c54" stop-opacity="0" />
                        </linearGradient>
                      </defs>
                    ...
                    <div class="sr-axis">
                      <span>turn 1</span><span>{{ report.totals.turns }} turns</span>
                    </div>
                  </div>
                  <div v-else class="empty">
                    No per-turn trace — session logs not found on this machine (totals
                    shown from run records).
                  </div>

                  <div class="sr-sec-head">
                    <h2>Tool calls by type</h2>
                    <span class="muted" style="margin-left: auto; font-size: 12px"
                      >{{ report.totals.tool_calls }} calls ·
                      {{ report.tools_by_type.length }} tools</span
                    >
                  </div>
                  <div v-if="report.tools_by_type.length" class="sr-tools">
                    <div
                      v-for="t in report.tools_by_type"
                      :key="t.name"
                      class="sr-tool-row"
                      :class="t.category">
                      <span class="sr-tool-name">{{ t.name }}</span>
                      <span class="sr-tool-bar"
                        ><i :style="{ width: (t.count / toolMax) * 100 + '%' }"></i
                      ></span>
                      <span class="sr-tool-count">{{ t.count }}</span>
                    </div>
                    <div class="sr-tlegend">
                      <span><i class="orchestration"></i>orchestration</span
                      ><span><i class="coordination"></i>coordination</span>
                      <span><i class="io"></i>file I/O</span
                      ><span><i class="mcp"></i>MCP</span
                  ...
                  <p class="muted" style="font-size: 11.5px; margin-top: 18px">
                    {{ report.pricing.note }} · Generate analysis ↗ copies a prompt to
                    author the full editorial report (narrative + recommendations) in your
                    AI tool.
                  </p>
                </div>
              </template>
            ...
            <template v-else>
              <div class="detail-head">
                ...
                <span class="grow"></span>
                <button v-if="!taskDetail.error" class="btn" @click="openReport">
                  Session report ↗
                </button>
                <span v-if="taskDetail.error" class="badge st-BLOCKED">{{
                  taskDetail.error
                }}</span>
              </div>
              ...
              <template v-if="!taskDetail.error">
                ...
                <div class="card row" style="flex-wrap: wrap">
                  <span class="badge" :class="'st-' + taskDetail.status">{{
                    prettyStatus(taskDetail.status)
                  }}</span>
                  ...
                  <span v-if="statusError" class="badge st-BLOCKED">{{
                    statusError
                  }}</span>
                  <span class="grow"></span>
                  <span v-if="taskDetail.next_agent" class="muted" style="font-size: 12px"
                    >next:
                    <span class="agent" :class="'agent-' + taskDetail.next_agent">{{
                      taskDetail.next_agent
                    }}</span></span
                  >
                  <button
                    v-if="taskDetail.next_agent === 'human'"
                    class="btn"
                    disabled
                    title="The task is BLOCKED — it needs a human decision, not an agent">
                  ...
                  <template v-else>
                    <label
                      ...
                      title="Git isolation: in-place = current branch; branch = tcgflow/&lt;TASK-ID&gt; in the same tree (ADR 0040); worktree = a dedicated worktree so tasks run in parallel (ADR 0043). No auto-merge."
                      >git:
                    ...
                    <label
                      class="chain-toggle"
                      title="Run to completion: after this role hands off, automatically run the next one (reviewer → tester → ingester) until the task is INGESTED, blocked, or bounces too often">
                    ...
                    <button
                      ...
                      {{
                        runState === 'running'
                          ...
                          : runState === 'paused'
                            ? 'Paused'
                            : 'Run ' + (taskDetail.next_agent || 'coder')
                      }}
                    </button>
                    <button
                      class="btn"
                      :disabled="runState === 'running'"
                      title="Review the task's branch (tcgflow/&lt;ID&gt;) and open a PR — pushes on confirm (ADR 0043)"
                      @click="reviewPr(selectedTask)">
                      ⑂ PR
                    </button>
                  </template>
                </div>
                ...
                <div v-if="prPanel" class="modal-backdrop" @click.self="closePr">
                  <div class="modal" style="max-width: 640px">
                    ...
                    <div v-else-if="prPanel.error" class="badge st-BLOCKED">
                      {{ prPanel.error }}
                    </div>
                    <template v-else-if="!prResult">
                      <div class="muted" style="font-size: 12px; margin: 6px 0">
                        branch <b class="mono">{{ prPanel.branch }}</b> → base
                        <b class="mono">{{ prPanel.base }}</b> ·
                        {{ prPanel.ahead }} commit(s) ahead<span
                          v-if="!prPanel.has_remote">
                          · no git remote</span
                        >
                      </div>
                      <div v-if="!prPanel.ahead" class="empty">
                        Nothing to push — no commits ahead of the base.
                      </div>
                      <template v-else>
                        <pre class="stream-pane" style="max-height: 180px"
                          >{{ prPanel.commits.join('\n') }}
...
{{ prPanel.diffstat }}</pre
                    ...
                    <div v-else style="margin-top: 8px">
                      <div v-if="prResult.pr_url">
                        ✓ PR opened:
                        <a :href="prResult.pr_url" target="_blank" rel="noopener">{{
                          prResult.pr_url
                        }}</a>
                      </div>
                      <div v-else-if="prResult.compare_url">
                        ✓ Pushed.
                        <a :href="prResult.compare_url" target="_blank" rel="noopener"
                          >Open the PR ↗</a
                        >
                      </div>
                      <div v-else>
                        ✓ Pushed <b class="mono">{{ prPanel.branch }}</b
                        >.
                      </div>
                    </div>
                  </div>
                ...
                <div v-if="wtPanel" class="modal-backdrop" @click.self="closeWorktrees">
                  <div class="modal" style="max-width: 640px">
                    ...
                    <div v-else-if="wtPanel.error" class="badge st-BLOCKED">
                      {{ wtPanel.error }}
                    </div>
                    <div v-else-if="!wtPanel.worktrees.length" class="empty">
                      No per-task worktrees on disk.
                    </div>
                      ...
                      <tbody>
                        <tr v-for="w in wtPanel.worktrees" :key="w.task_id">
                          ...
                          <td>
                            <span
                              class="badge"
                              :class="'st-' + (w.status || 'BLOCKED')"
                              >{{ w.status || 'no task' }}</span
                            >
                          </td>
                          <td style="text-align: right">
                            ...
                            <span
                              ...
                              class="muted"
                              style="font-size: 12px"
                              title="Only INGESTED/COMPLETED tasks are offered — the work must be folded into the wiki first"
                              >in flight</span
                            >
                          </td>
                        </tr>
                    ...
                    <div class="muted" style="font-size: 12px; margin-top: 8px">
                      Removing a worktree deletes only its working directory — the task
                      branch is kept, so nothing is lost.
                    </div>
                  </div>
                </div>
                ...
                <template v-if="runState !== 'idle'">
                  <div
                    ...
                    <span
                      class="badge"
                      :class="{
                        'st-IN_PROGRESS': runState === 'running',
                        'st-BLOCKED': runState === 'error' || runState === 'paused',
                        'st-COMPLETED': runState === 'done',
                      }"
                      >{{ runState }}</span
                    >
                    <span
                      v-if="runBranch"
                      class="badge soft mono"
                      :title="
                        'Running on git branch ' +
                        runBranch +
                        ' (ADR 0040 — no auto-merge)'
                      "
                      >⑂ {{ runBranch }}</span
                    >
                    <button
                      v-if="runState === 'running' || runState === 'paused'"
                      class="btn"
                      style="padding: 3px 11px; font-size: 12px"
                      @click="stopRun"
                      title="Stop this run">
                      ■ Stop
                    </button>
                  ...
                  <div v-if="liveTokens" class="chips" style="margin: 8px 0">
                    ...
                    <span class="badge soft"
                      >cache r {{ fmtNum(liveTokens.cache_read) }}</span
                    >
                    <span class="badge soft"
                      >cache w {{ fmtNum(liveTokens.cache_creation) }}</span
                    >
                  </div>
                ...
                <div v-if="roleEntries.length" class="card token-table">
                  <div class="token-row token-head">
                    <span>role</span><span>input</span><span>output</span
                    ><span>cache r</span><span>cache w</span>
                  </div>
                  <div v-for="[role, t] in roleEntries" :key="role" class="token-row">
                    <span class="agent" :class="'agent-' + role">{{ role }}</span>
                    <span>{{ fmtNum(t.input) }}</span
                    ><span>{{ fmtNum(t.output) }}</span
                    ><span>{{ fmtNum(t.cache_read) }}</span
                    ><span>{{ fmtNum(t.cache_creation) }}</span>
                  </div>
                  <div class="token-row token-total">
                    <span>total</span>
                    <span>{{ fmtNum(taskDetail.tokens.total.input) }}</span
                    ><span>{{ fmtNum(taskDetail.tokens.total.output) }}</span>
                    <span>{{ fmtNum(taskDetail.tokens.total.cache_read) }}</span
                    ><span>{{ fmtNum(taskDetail.tokens.total.cache_creation) }}</span>
                  </div>
                </div>
                <div v-else class="empty">
                  No tokens yet — only orchestrated <b>Run</b>s contribute
                  (manual/copy-prompt runs don't).
                </div>
                ...
                <div class="section">
                  Runs
                  <span
                    v-if="taskDetail.tokens.runs.length"
                    style="
                      font-weight: 400;
                      letter-spacing: 0;
                      text-transform: none;
                      color: var(--text-3);
                    "
                    >· click to read the transcript</span
                  >
                </div>
                <div v-if="!taskDetail.tokens.runs.length" class="empty">
                  No orchestrated runs yet.
                </div>
                <div
                  v-for="r in taskDetail.tokens.runs"
                  :key="r.run_id"
                  class="card row interactive"
                  ...
                  <button
                    class="btn"
                    style="padding: 3px 9px; font-size: 11px"
                    title="Session report for this run"
                    @click.stop="openReport(r.run_id)">
                    report
                  </button>
                  <button
                    class="btn"
                    style="padding: 3px 9px; font-size: 11px"
                    title="Git diff since this run started"
                    @click.stop="openDiff(r.run_id)">
                    diff
                  </button>
                  <button
                    v-if="r.session_id"
                    class="btn"
                    style="padding: 3px 9px; font-size: 11px"
                    :class="{ 'btn-copied': copiedKey === 'term' + r.run_id }"
                    title="Copy a command to resume this session in your own terminal"
                    @click.stop="copyResume(selected, r.session_id, 'term' + r.run_id)">
                    {{ copiedKey === 'term' + r.run_id ? '✓' : '⌥ term' }}
                  </button>
                  ...
                  <span
                    v-if="r.wiki_discovery && r.wiki_discovery.path"
                    class="badge"
                    :class="r.wiki_discovery.path === 'qmd' ? 'st-COMPLETED' : 'soft'"
                    :title="wikiDiscoveryTitle(r.wiki_discovery)"
                    >🔍 {{ r.wiki_discovery.path === 'qmd' ? 'qmd' : 'index-fallback'
                    }}<span v-if="r.wiki_discovery.redirects">
                      ⚠︎{{ r.wiki_discovery.redirects }}</span
                    ></span
                  >
                  ...
                  <span
                    v-if="r.write_attempts && r.write_attempts.count"
                    class="badge"
                    :class="
                      ['reviewer', 'tester'].includes(r.write_attempts.role)
                        ? 'st-BLOCKED'
                        : 'soft'
                    "
                    :title="
                      'This ' +
                      r.write_attempts.role +
                      ' run attempted ' +
                      r.write_attempts.count +
                      ' write(s) — ' +
                      (r.write_attempts.tools || 'Edit/Write') +
                      '. Observed, not blocked (ADR 0037): Edit/Write classify MEDIUM for every role, so separation of duties is not enforced by the gate.'
                    "
                  ...
                  <span
                    v-if="r.branch"
                    class="badge soft mono"
                    :title="'Ran on branch ' + r.branch + ' (ADR 0040 — no auto-merge)'"
                    >⑂ {{ r.branch }}</span
                  >
                  <span
                    class="badge"
                ...
                <div class="section">
                  ...
                  <span
                    style="
                      ...
                      color: var(--text-3);
                    "
                    >· resumes the latest run's session, read-only</span
                  >
                </div>
                <div v-if="!chatSession" class="empty">
                  No session to discuss yet — run this task first.
                </div>
                <template v-else>
                  <div v-if="chatMessages.length" class="chat-log">
                    <div
                      ...
                      <pre class="chat-text">{{
                        m.text || (chatBusy && i === chatMessages.length - 1 ? '…' : '')
                      }}</pre>
                    </div>
                  </div>
                  ...
                  <p class="muted" style="font-size: 11.5px; margin-top: 6px">
                    Read-only — the agent can inspect the project and answer, but can't
                    edit. For changes, run the task.
                  </p>
                </template>
                ...
                <pre
                  v-if="taskDetail.details_body && taskDetail.details_body.trim()"
                  class="plan"
                  >{{ taskDetail.details_body }}</pre
                >
                ...
                <div v-if="timelineNewest.length">
                  <div v-for="(e, i) in timelineNewest" :key="i" class="card">
                    <div class="sub" style="margin: 0">
                      <span class="badge soft" v-if="e.timestamp">{{
                        relTime(e.timestamp)
                      }}</span>
                      <span
                        class="badge"
                        :class="'agent-' + (e.author || 'human')"
                        style="background: var(--surface-2)"
                        >{{ e.author }}</span
                      >
                      <span v-if="e.via" class="badge jira">via {{ e.via }}</span>
                      <span v-if="e.status_from || e.status_to" class="badge soft"
                        >{{ e.status_from }} → {{ e.status_to }}</span
                      >
                      <span v-if="e.governance" class="badge st-BLOCKED"
                        >gov: {{ e.governance.risk }} {{ e.governance.decision }}</span
                      >
                    </div>
                    ...
                    <div
                      v-if="Array.isArray(e.files) && e.files.length"
                      class="chips"
                      style="margin-top: 6px">
                      <code v-for="f in e.files" :key="f">{{ f }}</code>
                    </div>
                    <ul
                      v-if="Array.isArray(e.validation) && e.validation.length"
                      style="
                        margin: 6px 0 0;
                        padding-left: 18px;
                        font-size: 12.5px;
                    ...
                    <div
                      v-if="Array.isArray(e.tags) && e.tags.length"
                      class="chips"
                      style="margin-top: 6px">
                      <span v-for="tg in e.tags" :key="tg" class="badge soft">{{
                        tg
                      }}</span>
                    </div>
                    <div
                      v-if="e.blocker"
                      class="badge st-BLOCKED"
                      style="margin-top: 6px">
                      blocked: {{ e.blocker.reason || e.blocker }}
                    </div>
                  </div>
                </div>
          ...
          <template v-else>
            <div class="page-head">
              <h1>
                {{ detail.config.name }}
                <span
                  v-if="detail.version.update_available"
                  class="badge st-IN_PROGRESS"
              ...
              <div class="meta">
                ...
                <span
                  class="badge soft"
                  title="Jira status cache — refresh with /tcgflow-sync-jira"
                  >Jira: {{ relTime(detail.jira_synced) }}</span
                >
                <span
                  v-if="projectBudget != null"
                  class="badge"
                  :class="overBudget ? 'st-BLOCKED' : 'soft'"
                  :title="
                    overBudget
                      ? 'Over the configured spend budget'
                      : 'Est. spend vs budget'
                  "
                  >spend {{ fmtUsd(projectSpend) }} / ${{ projectBudget
                  }}{{ overBudget ? ' ⚠' : '' }}</span
                >
              </div>
            </div>
            ...
            <div class="tabs">
              <button
                class="tab"
                :class="{ active: projectTab === 'overview' }"
                @click="projectTab = 'overview'">
                Overview
              </button>
              <button
                class="tab"
                ...
                Tasks <span class="tab-n">{{ detail.tasks.length }}</span>
              </button>
              <button
                class="tab"
                :class="{ active: projectTab === 'wiki' }"
                @click="projectTab = 'wiki'">
                Wiki
              </button>
              <button
                class="tab"
                :class="{ active: projectTab === 'governance' }"
                @click="projectTab = 'governance'">
                Governance
              </button>
              <button
                class="tab"
                :class="{ active: projectTab === 'timesheet' }"
                @click="projectTab = 'timesheet'">
                Timesheet
              </button>
              <button
                class="tab"
                :class="{ active: projectTab === 'tools' }"
                @click="projectTab = 'tools'">
                Tools
              </button>
              <button
                class="tab"
                :class="{ active: projectTab === 'settings' }"
                @click="projectTab = 'settings'">
                Settings
              </button>
            </div>
            ...
            <template v-if="projectTab === 'overview'">
              <template v-if="detail.config.projects && detail.config.projects.length">
                ...
                <div class="chips" style="margin-bottom: 8px">
                  <span
                    v-for="sp in detail.config.projects"
                    :key="sp.name"
                    class="badge soft">
                    <b>{{ sp.name }}</b
                    ><span class="muted" v-if="sp.stack"> · {{ sp.stack }}</span>
                  </span>
                </div>
              </template>
              ...
              <template v-if="projectAgents.length">
                ...
                <div class="agent-cards">
                  <div
                    v-for="a in projectAgents"
                    :key="a.role"
                    class="agent-card"
                    :class="'ac-' + a.role"
                    @click="filterByAgent(a.role)"
                    title="Show this agent's tasks">
                    <div class="ac-name agent" :class="'agent-' + a.role">
                      {{ a.role }}
                    </div>
                    <div class="ac-stat">
              ...
              <div class="section" style="display: flex; align-items: center; gap: 8px">
                ...
                <button
                  v-if="
                    detail.action_queue.length &&
                    detail.config &&
                    detail.config.orchestrator &&
                    detail.config.orchestrator.autopilot
                  "
                  class="btn btn-primary"
                  ...
                  @click.stop="startAll">
                  ▶▶ Start all ({{
                    detail.action_queue.filter((a) => !a.run_state).length
                  }})
                </button>
                <button
                  class="btn"
                  ...
                  @click.stop="openWorktrees">
                  ⑂ Worktrees
                </button>
              </div>
              <div v-if="!detail.action_queue.length" class="empty">
                Queue empty — nothing ready for an agent.
              </div>
              <div
                v-for="(a, i) in detail.action_queue"
                :key="a.task_id"
                class="card row interactive"
                @click="openTask(a)">
                <div class="grow">
                  <span class="task-id">{{ a.task_id }}</span
                  ...
                  <div class="sub">
                    ...
                    <span
                      >→
                      <span class="agent" :class="'agent-' + a.agent">{{
                        a.agent
                      }}</span></span
                    >
                    <span v-if="a.run_state" class="badge st-IN_PROGRESS"
                      >● {{ a.run_state }}</span
                    >
                    <span v-if="a.jira_status" class="badge jira"
                      >Jira: {{ a.jira_status }}</span
                    >
                    <span
                      v-if="a.jira_drift"
                      class="badge st-BLOCKED"
                      title="Workspace and Jira disagree on done-ness"
                      >⚠ drift</span
                    >
                  </div>
                </div>
                <button
                  ...
                  style="padding: 5px 11px"
                  title="Launch this agent now"
                  @click.stop="quickRun(selected, a.task_id, a.agent)">
                  ▶
                </button>
                <button
                  class="btn"
                  :class="{ 'btn-copied': copiedKey === 'q' + i }"
            ...
            <template v-else-if="projectTab === 'tasks'">
              <div class="tfilters">
                <div class="chips">
                  <button
                    v-for="b in ['active', 'completed', 'archive', 'all']"
                    :key="b"
                    class="chip"
                    :class="{ active: taskBucket === b }"
                    @click="taskBucket = b">
                    {{ b
                    }}<span v-if="b !== 'all'" class="chip-n">{{
                      bucketCounts[b] || 0
                    }}</span>
                  </button>
                </div>
                ...
                <span class="muted" style="font-size: 12px; margin-left: auto"
                  >{{ filteredTasks.length }} shown</span
                >
              </div>
              <div class="ttable">
                <div class="tr th">
                ...
                <div
                  ...
                  @click="openTask(t)">
                  <span class="tc-task"
                    ><b class="task-id">{{ t.id }}</b>
                    <span class="task-title">{{ t.title }}</span></span
                  >
                  <span
                    ><span class="badge" :class="'st-' + t.status">{{
            ...
            <template v-else-if="projectTab === 'wiki'">
              ...
              <div class="fresh-grid">
                <div class="card" :class="{ 'fresh-warn': detail.wiki.awaiting_ingest }">
                  ...
                  <div class="fs muted">
                    VALIDATED — done but not yet folded into the wiki
                  </div>
                </div>
                <div
                  class="card"
                  :class="{ 'fresh-warn': (detail.wiki.raw_pending || []).length }">
                ...
                <div class="card">
                  <div class="fv fv-sm">
                    {{
                      detail.wiki.wiki_last_edit
                        ? relTime(detail.wiki.wiki_last_edit).replace('synced ', '')
                        : '—'
                    }}
                  </div>
              ...
              <div
                v-if="(detail.wiki.raw_pending || []).length"
                class="card row"
                style="margin-top: 10px">
                <div class="grow chips">
                  <code v-for="f in detail.wiki.raw_pending.slice(0, 6)" :key="f">{{
                    f
                  }}</code>
                  <span
                    v-if="detail.wiki.raw_pending.length > 6"
                    class="muted"
                    style="font-size: 12px"
                    >+{{ detail.wiki.raw_pending.length - 6 }} more</span
                  >
                </div>
                <button
                  class="btn btn-primary"
                  title="Launch an ingester run that folds raw/ into the wiki"
                  @click="quickRun(selected, 'RAW-INGEST', 'ingester')">
                  ▶ Ingest raw now
                </button>
              </div>

              <div class="section">
                Recent activity · {{ detail.wiki.page_count }} pages
              </div>
              <div class="card" v-if="detail.wiki.recent_log.length">
                <div
                  v-for="(l, idx) in detail.wiki.recent_log"
                  :key="idx"
                  class="log-line">
                  {{ l }}
                </div>
            ...
            <template v-else-if="projectTab === 'governance'">
              ...
              <template v-if="detail.governance && detail.governance.rules.length">
                <div v-for="(r, idx) in detail.governance.rules" :key="idx" class="card">
                  {{ r }}
                </div>
              </template>
              <div v-else class="empty">
                No project-specific rules. Risk levels LOW · MEDIUM · HIGH · CRITICAL
                apply by default.
              </div>
            </template>
            ...
            <template v-else-if="projectTab === 'timesheet'">
              ...
              <div v-if="detail.timesheet && detail.timesheet.present" class="card row">
                ...
                <span
                  class="badge"
                  :class="detail.timesheet.submitted ? 'st-COMPLETED' : 'st-IN_PROGRESS'">
                  {{ detail.timesheet.submitted ? 'submitted' : 'draft' }}
                </span>
              </div>
              <div v-else class="empty">
                No timesheet this week. Run <code>generate-timesheet</code>.
              </div>
            </template>
            ...
            <template v-else-if="projectTab === 'tools'">
              ...
              <div class="card">
                <div class="chips" style="margin-bottom: 10px">
                  ...
                  <span
                    v-for="(on, name) in detail.tools_mcp.tools"
                    :key="name"
                    class="badge"
                    :class="on ? 'st-COMPLETED' : 'soft'"
                    >{{ name }} {{ on ? '✓' : '' }}</span
                  >
                </div>
                <div
                  class="chips"
                  v-if="detail.tools_mcp.mcp_recommended.length"
                  style="margin-bottom: 8px">
                  <span class="muted" style="font-size: 12px">MCP recommended</span>
                  <span
                    v-for="m in detail.tools_mcp.mcp_recommended"
                    :key="m"
                    class="badge soft"
                    >{{ m }}</span
                  >
                ...
                <div class="chips" v-if="detail.tools_mcp.mcp_optional.length">
                  ...
                  <span
                    v-for="m in detail.tools_mcp.mcp_optional"
                    :key="m"
                    class="badge soft"
                    >{{ m }}</span
                  >
            ...
            <template v-else-if="projectTab === 'settings'">
              ...
              <div class="card">
                <div
                  ...
                  <span
                    class="agent"
                    :class="'agent-' + role"
                    style="width: 120px; text-transform: capitalize"
                    >{{ role }}</span
                  >
                  <select v-model="settingsRoles[role]" class="fsel">
              ...
              <div class="card srow">
                <label
                  ...
                  Chain runs by default: after each role hands off, launch the next one
                  (coder → reviewer → tester → ingester) until INGESTED, BLOCKED, or the
                  bounce limit.
                </label>
              </div>
              ...
              <div class="card srow">
                ...
                <span class="muted" style="font-size: 12px"
                  >Default for new runs. <code>branch</code>/<code>worktree</code>
                  create/continue <code>tcgflow/&lt;TASK-ID&gt;</code> — no auto-merge,
                  you integrate it. <code>worktree</code> runs in its own dir so tasks go
                  parallel (ADR 0043). Overridable per run.</span
                >
              </div>
              ...
              <div class="card">
                <label
                  ...
                  Autopilot: every launched task runs in its own <b>worktree</b>,
                  auto-chains <b>planner → coder → reviewer</b>, and runs in parallel —
                  then waits for you to review &amp; open its PR (⑂ PR). No
                  tester/ingester, no auto-merge.
                </label>
                <div class="srow" style="margin-top: 6px">
                  <span style="width: 120px">max parallel</span>
                  <input
                    v-model="settingsMaxParallel"
                    class="fsearch"
                    type="number"
                    min="1"
                    step="1"
                    style="width: 130px" />
                  <span class="muted" style="font-size: 12px"
                    >Cap on concurrent worktree runs. Note: parallel autopilot fires many
                    real agent runs — mind the budget.</span
                  >
                </div>
              </div>
              ...
              <div class="card srow">
                ...
                <input
                  ...
                  style="width: 130px" />
                <span class="muted" style="font-size: 12px"
                  >Flags Home + this project when est. spend exceeds it.</span
                >
              </div>
              <div class="section">Pricing — list estimate (USD per 1M tokens)</div>
              <div class="ttable">
                <div class="tr th" style="grid-template-columns: 1fr 1fr 1fr 1fr 1fr">
                  <span>model</span><span>input</span><span>output</span
                  ><span>cache write</span><span>cache read</span>
                </div>
                <div
                  v-for="r in PRICING_TABLE"
                  :key="r.m"
                  class="tr"
                  style="grid-template-columns: 1fr 1fr 1fr 1fr 1fr">
                  <span>{{ r.m }}</span
                  ><span class="mono">${{ r.i }}</span
                  ><span class="mono">${{ r.o }}</span
                  ><span class="mono">${{ r.cw }}</span
                  ><span class="mono">${{ r.cr }}</span>
                </div>
              </div>
              <p class="muted" style="font-size: 11.5px; margin-top: 6px">
                Pricing is fixed in the tool (used for the Session Report + budget). Roles
                + budget persist to config.yaml.
              </p>
    ...
    <div v-if="pendingApproval" class="modal-backdrop">
      <div class="modal">
        <div class="section" style="margin-top: 0">
          ...
          <span
            class="badge"
            :class="pendingApproval.risk === 'CRITICAL' ? 'st-BLOCKED' : 'st-IN_PROGRESS'"
            >{{ pendingApproval.risk }}</span
          >
        </div>
        <div class="modal-row">
          <b>Action</b><code>{{ pendingApproval.action }}</code>
        </div>
        <div class="modal-row" v-if="pendingApproval.why">
          <b>Why</b><span>{{ pendingApproval.why }}</span>
        </div>
        <div
          class="modal-row"
          v-if="pendingApproval.files && pendingApproval.files.length">
          <b>Files</b
          ><span class="chips"
            ><code v-for="f in pendingApproval.files" :key="f">{{ f }}</code></span
          >
        </div>
        <div class="modal-row" v-if="pendingApproval.rollback">
          <b>Rollback</b><span>{{ pendingApproval.rollback }}</span>
        </div>
        <label
          v-if="pendingApproval.risk === 'CRITICAL'"
          class="modal-row"
        ...
        <div class="modal-actions">
          ...
          <button
            class="btn btn-primary"
            :disabled="pendingApproval.risk === 'CRITICAL' && !criticalAck"
            @click="decide('approve')">
            Approve
          </button>
        </div>
        <p class="muted" style="font-size: 11.5px; margin: 10px 0 0">
          Deny is non-fatal — the run records the action deferred to human and continues
          with what it can.
        </p>
      </div>
    </div>
    ...
    <div v-if="runView || runLoading" class="modal-backdrop" @click.self="closeRun">
      <div class="modal" style="width: min(840px, 94vw)">
        <div
          ...
          Run transcript
          <span
            v-if="runView && runView.role"
            class="agent"
            :class="'agent-' + runView.role"
            style="text-transform: none"
            >{{ runView.role }}</span
          >
        </div>
        ...
        <template v-else-if="runView && !runView.error">
          <div class="chips" style="margin-bottom: 10px">
            ...
            <span
              v-if="runView.state"
              class="badge"
              :class="runView.state === 'done' ? 'st-COMPLETED' : 'soft'"
              >{{ runView.state }}</span
            >
          ...
          <div
            ...
            <div
              ...
              <span style="text-transform: none; letter-spacing: 0"
                >· resume this run on the CLI</span
              >
            </div>
            <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap">
              <code
                ...
                >{{ runView.session_id }}</code
              >
              <button
                class="btn"
                style="padding: 3px 9px; font-size: 11px"
                :class="{ 'btn-copied': copiedKey === 'rv-id' }"
                title="Copy the session id"
                @click="copyText(runView.session_id, 'rv-id')">
                {{ copiedKey === 'rv-id' ? '✓ copied' : 'copy id' }}
              </button>
              <button
                class="btn"
                style="padding: 3px 9px; font-size: 11px"
                :class="{ 'btn-copied': copiedKey === 'rv-cmd' }"
                title="Copy: cd <project> && claude --resume <id>"
                @click="copyResume(runViewProject, runView.session_id, 'rv-cmd')">
                {{ copiedKey === 'rv-cmd' ? '✓ copied' : '⌥ resume cmd' }}
              </button>
            </div>
            <div
              class="mono"
              style="
                ...
                word-break: break-all;
              ">
              claude --resume {{ runView.session_id }}
            </div>
          </div>
          <pre class="stream-pane" style="max-height: 62vh">{{
            runView.transcript || '(empty transcript)'
          }}</pre>
        </template>
        <div v-else class="empty">Transcript not found.</div>
        <div class="modal-actions">
          <button class="btn" @click="closeRun">Close</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="diffView || diffLoading" class="modal-backdrop" @click.self="closeDiff">
      <div class="modal" style="width: min(900px, 95vw)">
        <div class="section" style="margin-top: 0">
          Run diff
          <span
            v-if="diffView && diffView.run_id"
            class="mono muted"
            style="font-size: 12px"
            >{{ diffView.run_id.slice(0, 8) }}… (since run start)</span
          >
        </div>
        <p v-if="diffLoading" class="muted">Loading…</p>
        <template v-else-if="diffView">
          <p v-if="diffView.note" class="muted" style="font-size: 12.5px">
            {{ diffView.note }}
          </p>
          <pre v-if="diffView.diff" class="stream-pane" style="max-height: 64vh">{{
            diffView.diff
          }}</pre>
          <div v-else-if="!diffView.note" class="empty">
            No changes since this run started.
          </div>
        </template>
        <div class="modal-actions">
          <button class="btn" @click="closeDiff">Close</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="showInbox" class="modal-backdrop" @click.self="showInbox = false">
      <div class="modal" style="width: min(640px, 94vw)">
        <div class="section" style="margin-top: 0">
          Approval inbox
          <span class="badge" :class="inbox.length ? 'st-BLOCKED' : 'st-COMPLETED'"
            >{{ inbox.length }} pending</span
          >
        </div>
        <div v-if="!inbox.length" class="empty">
          Nothing waiting for you. Chains run unattended until something HIGH/CRITICAL
          needs a decision.
        </div>
        <div v-for="a in inbox" :key="a.approval_id" class="card">
          <div class="sub" style="margin: 0 0 6px">
            <span
              class="badge"
              :class="a.risk === 'CRITICAL' ? 'st-BLOCKED' : 'st-IN_PROGRESS'"
              >{{ a.risk }}</span
            >
            <span v-if="a.task_id" class="task-id">{{ a.task_id }}</span>
            <span class="mono muted" style="font-size: 11px"
              >{{ (a.run_id || '').slice(0, 8) }}…</span
            >
          </div>
          <code
            style="
            ...
            >{{ a.action }}</code
          >
          <p v-if="a.why" class="muted" style="font-size: 12.5px; margin: 0 0 6px">
            {{ a.why }}
          </p>
          <div v-if="a.files && a.files.length" class="chips" style="margin-bottom: 6px">
            <code v-for="fp in a.files" :key="fp">{{ fp }}</code>
          </div>
          <p v-if="a.rollback" class="muted" style="font-size: 12.5px; margin: 0 0 6px">
            <b>Rollback:</b> {{ a.rollback }}
          </p>
          <label
            v-if="a.risk === 'CRITICAL'"
            style="
              display: flex;
              align-items: center;
              gap: 6px;
              ...
              cursor: pointer;
            ">
            <input type="checkbox" v-model="inboxAck[a.approval_id]" /> I acknowledge the
            rollback plan (required for CRITICAL)
          </label>
          <div style="display: flex; gap: 8px; justify-content: flex-end">
            <button class="btn" @click="decideInbox(a, 'deny')">Deny</button>
            <button
              class="btn btn-primary"
              :disabled="a.risk === 'CRITICAL' && !inboxAck[a.approval_id]"
              @click="decideInbox(a, 'approve')">
              Approve
            </button>
          </div>
        </div>
        <div class="modal-actions">
          <button class="btn" @click="showInbox = false">Close</button>
        </div>
      </div>
    </div>
...
.token-row > span:not(:first-child) {
  text-align: right;
}
...
.tr {
  display: grid;
  grid-template-columns: minmax(0, 1fr) 132px 96px 88px auto;
  align-items: center;
  gap: 12px;
  padding: 10px 16px;
}
...
.fresh-warn {
  border-color: #f5b03155;
}
.fresh-warn .fv {
  color: var(--amber);
}
.nav-item.urgent .label {
  color: var(--coral);
  font-weight: 600;
}
@media (max-width: 760px) {
...
.sr-hero {
  ...
  flex-wrap: wrap;
  padding-bottom: 18px;
  border-bottom: 1px solid var(--srline);
}
.sr-tokens-processed {
  font-size: 34px;
  font-weight: 700;
...
.sr-n {
  ...
  font-weight: 700;
  line-height: 1;
  background: linear-gradient(180deg, #fff, var(--amber2));
  -webkit-background-clip: text;
  background-clip: text;
  color: transparent;
}
...
.sr-metrics {
  ...
  grid-template-columns: repeat(5, 1fr);
  gap: 1px;
  background: var(--srline);
  border: 1px solid var(--srline);
  border-radius: 12px;
  overflow: hidden;
  margin-top: 20px;
}
...
.sr-tcard.hot .v {
  background: linear-gradient(120deg, var(--amber2), var(--coral2));
  -webkit-background-clip: text;
  background-clip: text;
  color: transparent;
}
...
.sr-trace {
  border: 1px solid var(--srline);
  border-radius: 12px;
  background: linear-gradient(180deg, var(--srpanel), var(--srbg));
  padding: 16px;
}
.sr-trace svg {
  width: 100%;
...
.sr-tool-bar i {
  ...
  height: 100%;
  border-radius: 4px;
  background: var(--srmuted);
}
.sr-tool-row.orchestration i {
  background: #9d8cf0;
}
.sr-tool-row.coordination i {
  background: #46c6e0;
}
.sr-tool-row.io i {
  background: #5ee0c2;
...
.sr-tlegend .orchestration {
  background: #9d8cf0;
}
.sr-tlegend .coordination {
  background: #46c6e0;
}
.sr-tlegend .io {
  background: #5ee0c2;
