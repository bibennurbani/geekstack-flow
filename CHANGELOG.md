...
- **Internal deepenings + test seams** — one task-header reader (kills a byte-identical parse); characterization tests for the `init.js` project-detection cascade; the Cockpit server's request handler is extracted as a named `handleRequest`, so the full request→validation→dispatch map is exercised by in-memory request/response doubles (no socket) — including the ADR-0008 invariant that no client path can one-click-approve a CRITICAL action without acknowledging the rollback plan. Three more seams from the architecture review's structural tail: **one git seam** (`git.cjs` — `head`/`diffSince` with an injectable exec; the per-run diff endpoint is now unit-tested without a real repo), **config.yaml parse/edit primitives** (`config-fields.cjs` — `block`/`blockScalar`/`blockHasTrue`/`editBlockLine`, ending four copies of the block-scope idiom across `read.cjs`/`run.cjs`; edits stay surgical so the template's comments survive), and the **pure init plan** (`computeInitPlan`/`renderConfigYaml`/`initVars` lifted out of `init.js` `main()` — the single-vs-multi decision and config render are now testable without prompts or disk). Two suites now: `npm test` (node --test — server/CLI) and `npm --prefix ui test` (vitest — SPA).
...
- **Full session ID in the transcript modal** — the run-transcript modal shows the run's complete Claude session id (click-to-select), with a "copy id" button and a "⌥ resume cmd" button that copies `cd "<project>" && claude --resume <id>`, so a run can be picked up in your own terminal. (It used to truncate the id to 8 chars — not enough to `--resume`.)
