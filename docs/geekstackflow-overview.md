...
| **Version** | v0.5.1 — fixes the Cockpit settings save |
...
| **Maturity** | 6 releases (0.1.0 → 0.5.1), **45 ADRs**, **346** passing tests |
...
Every task is **exactly two files** — non-negotiable, never per-subtask files (the sole exception is `{ID} web-test-summary.md`, written only when a browser web test ran — ADR 0041):
...
| Term | Definition |
|------|------------|
| **geekstackflow / tcgflow** | Creative GeekStack Flow — a zero-dependency Node CLI (v0.5.1, two bin names) that scaffolds and orchestrates a file-based AI-development workflow. |
