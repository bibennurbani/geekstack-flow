...
- **Claude Code `Workflow` tool + `agentType: 'Explore'` + structured-output `schema` does NOT compose.** Subagents finish without calling StructuredOutput. Workaround: drop `agentType: 'Explore'` and use the default workflow subagent, or drop the `schema` and parse free-form text manually. Inline parallel reads via `Bash`/`Read` are the cheaper alternative for one-shot inventories.
