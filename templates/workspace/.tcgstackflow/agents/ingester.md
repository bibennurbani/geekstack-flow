...
**The Ingester is the wiki's only writer — one exception.** `frame-feature` may write `wiki/domain.md`, `wiki/architecture.md` and `wiki/adr/` during an interactive discovery session, per-page approved, wrapped in `<!-- intent: {ID} (not shipped) -->` markers and logged to `wiki/log.md`; the Ingester reconciles those blocks at `INGESTED` (ADR 0045). It takes a Raw source — a `VALIDATED` task, a batch of files in `raw/`, or an MCP-driven investigation — and folds it into the LLM-wiki: updating affected pages, proposing new pages for approval, flagging contradictions, and appending one entry to `wiki/log.md`.
...
2. **Plan.** Identify (a) which existing pages will be touched, (b) which new pages are candidates, (c) any contradictions with current wiki content. Walk the wikilink graph one hop from each affected page. **Dedup before proposing a new page** (`qmd vsearch` the topic; append when it's a facet of an existing page, mint only a distinct first-class concept — never a near-duplicate). **Coverage map (task ingest):** union the task log's `### ENTRY START` `files:` lists and ensure every touched file resolves to a Modified/Created page or an explicit `no wiki impact:` line in the Decision — so a real change is never silently un-documented. **Discovery intent blocks (task ingest):** if the task began as `/tcgflow-new-feature`, run `grep -rn "intent: {ID}" wiki/` — unanchored, so it matches both `<!-- intent: {ID} (not shipped) -->` and its closer — and fold every page it finds into the affected-pages set (ADR 0045). They are claims the wiki already makes on the task's behalf; they get resolved at step 4 like any other page update. (Full rules in the `ingest` skill.)
3. **Draft the `log.md` entry FIRST** — before any page changes. Use the locked prefix and the structured-body shape (Context / Created / Modified / Deleted / Decision). The draft entry IS the plan; if the user wants changes to the plan, they edit this draft. Discovery intent blocks are reported like every other change — each page carrying one goes in **Modified**, an abandoned discovery ADR goes in **Deleted**, and **Decision** says which way each block went (kept, corrected, or dropped), so a page corrected at step 4 is a page the user saw in the plan first.
4. **Apply existing-page updates.** Update sections affected; bump frontmatter `updated:` on every page touched; preserve `[[wikilinks]]`. Mechanical updates (a file moved, a version bumped, a dependency renamed) flow without further approval. Semantic rewrites (changed conclusions, new architectural framing) show a proposed diff first. **Resolve discovery intent here (ADR 0045)** — for each block the step-2 grep found: shipped as framed → drop the two marker comments and keep the prose; shipped differently → correct the body to what actually shipped (Semantic path), then drop the markers; an ADR written during discovery carries `status: stub` → flip it to `status: current`, or, if the decision was abandoned, leave the file and take it through the deletion gate (step 5).
...
12. **Re-embed the wiki search index.** After applying page changes and finalising the log entry, run an incremental `qmd embed` so the qmd index reflects the new/changed pages — the Ingester owns wiki-search freshness, and discovery's intent blocks arrive un-embedded too, so readers rely on this step. This also refreshes the `docs/` collection. If qmd is unavailable, note it; `index.md` stays the fallback.
...
5. **Re-embed the wiki search index.** After applying page changes and finalising the log entry, run an incremental `qmd embed` so the qmd index reflects the new/changed pages — the Ingester owns wiki-search freshness, and readers rely on a fresh index. This also refreshes the `docs/` collection. If qmd is unavailable, note it; `index.md` stays the fallback.
