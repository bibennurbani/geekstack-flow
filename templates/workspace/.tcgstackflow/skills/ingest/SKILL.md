...
2. **Plan.** Use the `wiki-search` skill (qmd) to surface the pages the Raw touches, and walk `wiki/index.md` to place them in the Map of Content. Identify:
   ...
   - **Discovery intent blocks (task ingest only).** A task that began as `/tcgflow-new-feature` may have left speculative blocks in `wiki/domain.md`, `wiki/architecture.md` and `wiki/adr/` — written by `frame-feature` before anything shipped, each wrapped in `<!-- intent: {ID} (not shipped) -->` … `<!-- /intent: {ID} -->` (ADR 0045). Run `grep -rn "intent: {ID}" wiki/` — unanchored, so it matches the opener with its `(not shipped)` suffix as well as the closer — and fold every page it finds into the affected-pages set. These blocks are claims the wiki is already making on the task's behalf; they get resolved at step 4 like any other page update.
3. **Draft the `log.md` entry FIRST.** Use the locked prefix `## [YYYY-MM-DD] ingest | {short title}`. Include:
   ...
   - **Discovery intent blocks** — report them like every other change: each page carrying one goes in **Modified** (its block kept or corrected), an abandoned discovery ADR goes in **Deleted**, and **Decision** says which way each block went — kept, corrected, or dropped. The discovery session's own `frame-feature {ID}` entry is already in `log.md`; this entry is what closes it. A page corrected here is a page the user saw in the plan first.
...
4. **Apply existing-page updates** (mechanical or semantic):
   ...
   - **Discovery intent (ADR 0045)** — resolve every block the step-2 grep found, and bump the page's `updated:`:
     - **What shipped matches what was framed** — drop the two marker comments and keep the prose. The claim is observed fact now, and reads like every other line on the page.
     - **What shipped differs** — correct the page body to what actually shipped (via the Semantic path), then drop the markers. The framing was a guess; the diff is the record.
     - **An ADR written during discovery** carries `status: stub` in its frontmatter — flip it to `status: current` once the decision shipped. If the decision was abandoned rather than implemented, leave the file untouched here and take it through the deletion gate (step 5); an abandoned decision left in `wiki/adr/` reads as one the project made.
...
- **Leaving intent markers behind.** An `<!-- intent: {ID} (not shipped) -->` … `<!-- /intent: {ID} -->` block still in the wiki after `{ID}` reaches `INGESTED` is a claim about a feature nobody confirmed shipped. `grep -rn "intent: {ID}" wiki/` at step 2 is what finds them; nothing lints for it — this ingest is the only thing that clears it.
