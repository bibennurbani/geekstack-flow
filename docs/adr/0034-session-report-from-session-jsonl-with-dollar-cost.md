...
- **Pricing drift:** list prices are estimates kept in two mirrored tables that must be updated together — `session-report.cjs` `PRICING` (the canonical server table) and `App.vue` `PRICE`/`PRICING_TABLE` (the Home est-spend hero, budget alerts, and the rendered pricing table); effective rates differ. The report always says so. *(Folding the SPA copies back onto a single server-exposed table is a known cleanup.)*
