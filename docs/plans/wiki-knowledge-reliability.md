...
**WK-2 · R · ✅ DONE · Install the pull-hook in `init`** — *the highest-leverage "doesn't forget" mechanism, was unwired.* (Shipped: `init.js main()` offers `installHooks` — default yes, guarded on `.git`; `upgradeWorkspace` nudges an unwired repo; `tcgflow-init` SKILL step 6. Verified end-to-end on a temp repo.) `init.js main()` never calls `installHooks()`; wiring needs a separate, undocumented `geekstackflow hooks .`. So fresh workspaces **silently never capture upstream pulls.** Add an `askYesNo('Install the git pull-digest hook?', true)` → `installHooks(target)` in `main()` (guarded on `.git` existing; advise the command when not yet a git repo); reminder in `upgradeWorkspace` when `.git/hooks/post-merge` lacks the marker; add a step to `tcgflow-init/SKILL.md` (the AI follows the skill, not the README). *~10 lines, reuses a hardened function.*

**WK-3 · R · ✅ DONE · Reconcile pending ingest on startup** — *closes the loop for human-validated tasks.* (Shipped: pure `pendingIngestPlan` + `reconcilePendingIngest` in `index.cjs`, run after the gate is wired, opt-in via `auto_ingest_on_pull`, startup-only; 3 unit tests.) `awaiting_ingest` is a passive badge; nothing enqueues an ingester when a task hits `VALIDATED` (no watcher/scheduler in `ui/server`). Add `reconcilePendingIngest(ws)` called from `reconcileAllProjects` (index.cjs:432), gated on `auto_ingest_on_pull`: for each `VALIDATED` task with no in-flight ingester run → `runManager.enqueue(...,'ingester',{chain:false})`; for a non-empty `raw/` inbox → one `RAW-INGEST` run; idempotent against `runManager.list()`. **Startup-reconcile only — no timer** (each enqueue is a billable run; the pull-hook + one-click buttons cover the steady state).
...
**✅ WK-6/7/8 DONE (data layer; UI rendering deferred to a browser smoke).** `read.cjs` `stalePagesFor()` computes per-page staleness deterministically — a page a *newer* ingest named but never bumped, preferring `verified:` over `updated:` — exposed as `wiki.stale_pages` per project (via `readWiki`) and a `stale_wiki` boolean on `buildProjectsList` (the per-project Home flag CONTEXT.md:128 promised, now real). Optional `verified:` field added to the ingest frontmatter schema (Ingester-stamped — single-writer preserved). 2 unit tests in `read-cjs.test.cjs`; 118/118. The Cockpit *card* that renders `stale_pages` + the Home `stale_wiki` badge is App.vue work — held for a browser smoke, like Cockpit CK-3.
...
**✅ WK-9 … WK-14 DONE** — 6 skill/template files edited (ingest, lint-wiki, wiki-search, ingester, adr/README, index.md). Adversarially reviewed across 4 dimensions (consistency / doctrine / goal-fit / cross-file coherence): **0 high issues**; the 2 medium + 2 low coherence findings were folded in (lint #8 ADR `updated`-carve-out; detector 12 ↔ ingest Decision-naming; append-vs-mint threshold wording; detector #4 ↔ append-vs-mint). The doctrine reviewers confirmed ADR 0007's report-only/gated invariant and ADR 0003's lightweight-ADR ethos survive. **WK-15 deferred** (lint determinism/cadence is lower-priority hardening; the `docs/` re-embed it called for is already covered by WK-1).
