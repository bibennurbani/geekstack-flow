...
- **`geekstackflow doctor`** — ✅ **DONE.** New CLI subcommand (`init.js`): for every registered project (+ the cwd workspace), checks against live qmd that each declared `wiki_search` collection is registered, points at **THIS** project's path, and the index has embeddings. Read-only; exits non-zero on any problem. Pure parsers/diagnosis unit-tested (`test/doctor.test.cjs`, 9 tests). Closes the "declared ≠ realized" gap.
  - **⚠ LIVE FINDING (2026-07-14):** running `doctor` proved the multi-project collision is **real and active** — the global `wiki` collection pointed at `INX`, so other projects had been searching **INX's wiki**, not their own.
  - **✅ FIXED — ADR 0038 (project-local index).** Chose the **project-local `.qmd` index** (`qmd init`) over renaming collections, because it keeps `-c wiki`/`-c docs` **stable** (no skill/agent changes) while isolating each project (verified: walk-up from cwd resolves the local index; markers never leak to global). Shipped: `.qmd/` gitignored (`init.js`); `/tcgflow-init` runs `qmd init` first; `/tcgflow-upgrade` re-creates the local index (the migration for existing workspaces); `wiki-search` skill + config template note the local index; **`doctor` now runs qmd per-project cwd and flags a missing `.qmd/`** (the pre-fix state). `init.js` the script stays dependency-free. Remediation for existing workspaces is an AI/HIGH step surfaced by `doctor` + `/tcgflow-upgrade`.
