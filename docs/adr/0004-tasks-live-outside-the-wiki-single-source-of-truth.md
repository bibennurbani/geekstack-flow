...
A real working structure observed in the wild has its tasks folder (`.taskRef/`) and its wiki folder (`ai-mem/docsRef/`) as *siblings*, not nested. Tasks are operational artifacts with their own lifecycle (active → completed → archive); the wiki is the distilled knowledge layer. Keeping them sibling makes that lifecycle explicit and lets `.tcgstackflow/tasks/` be moved, renamed, or replaced without touching the wiki. That same real workspace also shows an unintended duplication — the tasks folder appearing both at the project root *and* mirrored inside the memory folder — that we explicitly do not replicate: each task folder has exactly one home.
