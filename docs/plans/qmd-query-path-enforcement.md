...
>
> **Status:** IN_REVIEW · **Planner/Coder:** claude · **Source:** grill-with-docs session (2026-07-14) → ADR 0037. **Decides:** ADR 0037 (amends 0030).

## 0. Build progress

| Subtask | Size | State | Notes |
|---|---|---|---|
| QPE-0 ADR 0037 + CONTEXT.md "mandatory" disambiguation | S | ✅ | ADR written; CONTEXT.md Wiki-search term split into install-time vs query-time. |
| QPE-1 gate: wiki-discovery classify + per-run `qmdSeen`/`qmdAbsent` state + soft-deny redirect (+tests) | M | ✅ | `classifyWikiDiscovery`/`isQmdInvocation` in `governance-classify.cjs`; `decide()` + async qmd boot-check + redirect in `governance-mcp.cjs`; `Grep` dropped from `allowedTools`. Fail-**open**. **9 unit tests.** |
| QPE-2 run-record `wiki_discovery` block (serialize/parse, +tests) | S | ✅ | `read.cjs` serialize/parse (path/reason/queries/redirects), sibling of `embed`. **3 round-trip tests.** |
| QPE-3 Cockpit badge | S | ◐ | `read.cjs` projection (runs list + detail) + `App.vue` badge + `wikiDiscoveryTitle`. **Build clean; needs a live-Cockpit browser smoke** (no headless browser here). |
...
**Verified:** `node --test` **182/182** (167 + 15 new); `npm --prefix ui run build` clean. `ui/dist` is not git-tracked (built on install).

**Deferred (one runtime seam):** the end-to-end gate — native `Grep` routing through the permission tool, `qmdSeen` flipping on a live `qmd query`, the redirect reaching the agent, and the discovery telemetry populating the badge — can only be confirmed against a **live Cockpit run** (no headless browser/gated-claude here, per working-style). The *logic* is unit-tested; the *wiring* needs a smoke.
