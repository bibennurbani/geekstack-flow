...
- **Index freshness = the writer re-indexes.** An incremental `qmd embed` of all collections runs at `init`/`upgrade` and as the **Ingester's final step** (the Ingester is the wiki's reconciling writer, touching 10–15 pages per ingest; discovery's intent writes are the one bounded exception and re-index themselves — ADR 0045). `docs/` edits made by the Coder during a task are picked up at the next ingest; the periodic Lint is the backstop for `docs/` changed outside any task flow.
