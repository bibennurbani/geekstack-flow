...
6. **git isolation (ADR 0040/0043)** — a `git:` select next to the chain toggle picks how the run touches git: `in-place` (the current branch, default), `branch` (create/continue `tcgflow/<TASK-ID>` in the same working tree), or `worktree` (a dedicated git worktree so tasks can run in **parallel**). It is keyed on the task, so a chain's downstream roles **continue on the branch/worktree the coder created**. There's **no auto-merge** — you review the diff and integrate it yourself. Set the per-project default in **Settings**.
7. **Autopilot (ADR 0043)** — flip `orchestrator.autopilot` on and every task you launch runs in its own **worktree**, auto-chains **planner → coder → reviewer**, and runs **in parallel** (capped by `max_parallel`). Press **▶▶ Start all** to launch the whole queue, then come back and **⑂ PR** each finished task — a dialog (and `geekstackflow pr <TASK-ID>`) shows what would be pushed, then pushes the branch and opens a **draft PR** (via `gh`, else a compare URL). Push/PR stay human-invoked — nothing hits the remote unattended.
...
| `/tcgflow-web-test [ID] [env/URL]` | Browser verification of a task or your uncommitted change (Claude in Chrome) — happy path + required edge cases, writes `{ID} web-test-summary.md` with repro steps per bug. Interactive only |
...
| `web-test` | tester / standalone | Drive a real browser for UI criteria the suites can't settle — edges required, summary file with bug repros |
...
- **refreshes tool-owned files** — `tcgflow-*` commands (in the workspace *and* `~/.claude/skills/`), agent profiles, the workspace README, and the **tool-owned head of each tool adapter** — in `tools/` *and* the copy at your project root, which is the file your AI actually reads (ADR 0042). Your below-marker overrides are kept verbatim; any drifted file is backed up to `{name}.bak` first,
...
**Your customizations are never clobbered** — `governance.md`, `config.yaml`, existing skills, and everything below each adapter's "Edit below this line" marker are left for manual merge (and an edited adapter head is backed up before it is rebuilt). The drift report (re-runnable anytime with `geekstackflow drift /path/to/project`) tells you which of those drifted from the new templates, so the merge is targeted, not guesswork. Restart Claude Code afterward to pick up refreshed slash commands. (ADR 0021.)
...
├── docs/adr/               # 47 Architecture Decision Records
...
└── templates/
    ...
    │   ├── runs/          # orchestrated run records, {task-id}/{run-id}.md (schema 8)
    │   ├── raw/  prompts/
...
- **[docs/adr/](docs/adr/)** — 47 Architecture Decision Records. Highlights: scope ladder (0001), manual cross-tool handoff (0002), wiki structure (0003), two-file tasks (0004), skill/agent/adapter model (0005), governance (0008), the Cockpit & Orchestrator design (0020–0027), tester role (0028), Jira-via-cache (0029), qmd-mandatory wiki search (0030), refactorer role + cleanup-pass doctrine (0031), **Cockpit becomes the Orchestrator — read-only retired (0032)**, per-run token capture (0033), $-cost session reports (0034), per-tool runner-adapter seam + fidelity tiers (0035), deterministic qmd re-embed after ingest (0036), qmd discovery-path recording + project-local index + deterministic wiki-structure check (0037–0039), per-run git isolation in-place/branch/worktree (0040), worktree autopilot → parallel runs + PR command (0043), browser web-test as a Tester skill, interactive-only (0041), `upgrade` owns the adapter head + root copies (0042), discovery stage + intent writes to the wiki (0045), eval is a manual skill and only the capture is automatic (0046), delegation is measured before it is granted (0047).
