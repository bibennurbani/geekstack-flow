...
2. **Collections registered in THIS project's local index?** The workspace has a **project-local qmd index** (`.qmd/`, created by `qmd init` — ADR 0038), so collection names are scoped to this project and `-c wiki` is unambiguously *your* wiki even when other geekstackflow projects exist on the machine. Run `qmd collection list` **from the workspace root** (qmd walks up to find `.qmd`). It expects a `wiki` collection (`.tcgstackflow/wiki/`) and, when the project has a `docs/` directory, a `docs` collection. If `.qmd/` is missing, run `qmd init` first (else you'd register into the shared global namespace); register any missing collection with `qmd collection add <path> --name <name> --mask "*.md"`. `init` also runs `qmd context add qmd://<name> "<desc>"` — a one-line description that improves retrieval; set one if missing. (`geekstackflow doctor` verifies each collection resolves to this project.)
